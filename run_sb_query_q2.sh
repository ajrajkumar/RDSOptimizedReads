...
select 'BEGIN:'||extract (epoch from current_timestamp::timestamp(0));      
EOF
# Running queries
echo "Proceeding to run queries"
echo "Running query"
nohup pgbench -h $PGHOST -p $PGPORT -U $PGUSER $BMDB -f $BASE/q.sql -n -P 900 -t 1 -c $NUM_CONCURRENT_SESSIONS &
wait
...
select 'END:'||extract (epoch from current_timestamp::timestamp(0));      
WITH q AS (
    SELECT
        calls,
        query,
